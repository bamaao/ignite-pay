...
async fn test_app_state_creation() {
    ...
    assert_eq!(state.did_agent.mediator_did(), "did:test:mediator");
}
...
async fn test_message_store_enqueue_dequeue() {
    ...
    let msg = didcomm_mediator::storage::QueuedMessage {
        id: "msg-1".to_string(),
...
async fn test_mediate_request_response() {
    ...
    didcomm_mediator::protocols::coordinate_mediation::handle_mediate_request(
        &msg,
        &state,
        Some("did:test:alice"),
    )
...
async fn test_forward_queuing() {
    ...
    // Register Alice's key with the mediator
    state
        .keylist_store
        .add_key("did:test:alice", "did:test:alice#key-1")
        .await
    ...
    didcomm_mediator::protocols::routing::handle_forward(&msg, &state)
        .await
        .unwrap();
