...
pub async fn handle_status_request(
    ...
    let from = msg.from.as_deref().or(session_did).ok_or_else(|| {
        RouterError::Unauthorized("status-request requires a 'from' field".into())
    })?;
    ...
    let status_json =
        serde_json::to_string(&status).map_err(RouterError::Serialization)?;
...
pub async fn handle_batch_pickup(
    ...
    let from = msg.from.as_deref().or(session_did).ok_or_else(|| {
        RouterError::Unauthorized("batch-pickup requires a 'from' field".into())
    })?;
    ...
    let batch_json =
        serde_json::to_string(&batch).map_err(RouterError::Serialization)?;
