...
///
/// The client asks the router to start mediating for them.
...
pub async fn handle_mediate_request(
    ...
    let from = msg.from.as_deref().or(session_did).ok_or_else(|| {
        RouterError::Unauthorized("mediate-request requires a 'from' field".into())
    })?;
...
///
/// The client tells the router which recipient DIDs to route to them.
...
pub async fn handle_keylist_update(
    ...
    let from = msg.from.as_deref().or(session_did).ok_or_else(|| {
        RouterError::Unauthorized("keylist-update requires a 'from' field".into())
    })?;
    ...
    let updates = msg
        ...
        .ok_or_else(|| RouterError::Protocol("Missing 'updates' array in body".into()))?;
