...
async fn main() -> anyhow::Result<()> {
    ...
    let config_path = args.get(1).map(|s| s.as_str()).unwrap_or("config-provider.toml");

    tracing::info!("Loading config from {}", config_path);
    let config = ignite_pay_channel_service::config::Config::load(config_path)?;
    let bind_addr = config.bind_addr();

    let state = AppState::new(config, Role::Provider)?;
    let app = ignite_pay_channel_service::server::router::build_router(state);

    let listener = tokio::net::TcpListener::bind(&bind_addr).await?;
    tracing::info!("Channel Provider service listening on {}", bind_addr);
